...
├── src/
│   ├── app/
│   │   ├── (home)/                 # Trang chủ (Danh sách tài liệu, Templates, ...)
│   │   ├── documents/[documentId]/ # Giao diện soạn thảo tài liệu (Editor, Toolbar, ...)
│   │   └── api/liveblocks-auth/    # API xác thực quyền cho Liveblocks
│   ├── components/                 # Các UI component dùng chung (Dialogs, UI...)
│   ├── extensions/                 # Custom Extensions cho TipTap (Font size, Line height)
│   ├── hooks/                      # Các custom React hooks
│   ├── constants/                  # Các hằng số mặc định (ví dụ: margins.ts)
│   └── store/                      # Quản lý global state với Zustand
├── convex/                         # Mã nguồn Backend serverless (Convex)
│   ├── schema.ts                   # Định nghĩa Database Schema
│   ├── documents.ts                # API operations (Tạo, xóa, sửa, tìm kiếm)
│   └── auth.config.ts              # Cấu hình JWT cho Clerk <-> Convex
├── liveblocks.config.ts            # Cấu hình types TypeScript cho Liveblocks
